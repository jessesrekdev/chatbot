// Advanced Markdown Renderer with Syntax Highlighting
(() => {
  ...
  window.parseMarkdown = function(text) {
    ...
    // 1. Extract and store code blocks (must be first)
    html = html.replace(/```(\w+)?\n?([\s\S]*?)```/g, (match, lang, code) => {
      ...
      codeBlocks.push({
        placeholder,
        html: createCodeBlockHTML(codeId, language, highlightedCode)
      });
      return placeholder;
    });
    ...
    // 3. Bold + Italic (must be before individual bold/italic)
    ...
    // 9. Headers (must handle line-by-line)
    html = html.split('\n').map(line => {
    ...
    // 14. Line breaks (double newline = paragraph)
    html = html.replace(/\n\n+/g, '</p><p class="markdown-p">');
    html = '<p class="markdown-p">' + html + '</p>';

    // 15. Single line breaks
    html = html.replace(/\n/g, '<br>');
  ...
  console.log('📝 Advanced Markdown Renderer loaded with syntax highlighting!');
})();
